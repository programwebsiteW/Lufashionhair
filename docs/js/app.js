...
async function salvarPagamento(id,agendamentoId){
  ...
  try{if(id)await api.atualizarPagamento(id,dados);else await api.criarPagamento({...dados,agendamento_id:agendamentoId});fecharModal();await atualizarSistema();mostrarToast('Pagamento salvo.');}catch(err){mostrarToast(err.message);}
}
...
function formularioServico(s={}){const clientes=clientesCache.map(c=>`<option value="${c.id}">${escapar(c.nome)}</option>`).join('');abrirModal(s.id?'Editar serviço':'Novo serviço',`${!s.id?`<div class="campo"><label for="serv-modelo">Começar com um modelo</label><select id="serv-modelo"><option value="">Escolha ou preencha abaixo</option>${Object.entries(modelosServico).map(([k,v])=>`<option value="${k}">${v.nome}</option>`).join('')}</select></div>`:''}<div class="campo"><label for="serv-nome">Nome do serviço</label><input id="serv-nome" value="${escapar(s.nome||'')}" placeholder="Ex.: Progressiva"></div><div class="linha-campos"><div class="campo"><label for="serv-duracao">Tempo necessário</label><select id="serv-duracao">${opcoesDuracao(s.duracao_minutos||60)}</select></div><div class="campo"><label for="serv-preco">Valor</label><div class="campo-dinheiro"><span>R$</span><input id="serv-preco" inputmode="decimal" value="${valorCampo(s.preco)}" placeholder="Deixe em branco e digite"></div></div></div>${!s.id?`<label class="check"><input type="checkbox" id="serv-agendar"> Também agendar este serviço agora</label><div id="serv-agendamento-campos" class="destaque-form oculto"><strong>Dia, hora e cliente</strong><div class="linha-campos"><div class="campo"><label for="serv-data">Dia</label><input type="date" id="serv-data" value="${hojeISO()}"></div><div class="campo"><label for="serv-hora">Hora</label><input type="time" id="serv-hora"></div></div><div class="campo"><label for="serv-cliente">Cliente</label><select id="serv-cliente"><option value="">Escolha uma cliente</option>${clientes}</select></div></div>`:''}<p class="ajuda-campo">Se marcar “Também agendar”, o serviço será salvo e já aparecerá na Agenda.</p><div class="modal-botoes"><button class="btn-cancelar" onclick="fecharModal()">Voltar</button><button class="btn-confirmar" onclick="salvarServico(${s.id?Number(s.id):'null'})">Salvar</button></div>`);document.getElementById('serv-modelo')?.addEventListener('change',e=>{const m=modelosServico[e.target.value];if(m){document.getElementById('serv-nome').value=m.nome;document.getElementById('serv-duracao').value=m.duracao;}});document.getElementById('serv-agendar')?.addEventListener('change',e=>document.getElementById('serv-agendamento-campos').classList.toggle('oculto',!e.target.checked));document.getElementById('serv-nome').addEventListener('blur',e=>e.target.value=formatarNome(e.target.value));}
...
async function salvarServico(id){const precoTexto=document.getElementById('serv-preco').value.trim();const dados={nome:formatarNome(document.getElementById('serv-nome').value),duracao_minutos:Number(document.getElementById('serv-duracao').value),preco:precoTexto===''?null:parseDinheiro(precoTexto),ativo:true};const agendar=!id&&document.getElementById('serv-agendar')?.checked;const clienteId=agendar?document.getElementById('serv-cliente').value:null;const data=agendar?document.getElementById('serv-data').value:null;const hora=agendar?document.getElementById('serv-hora').value:null;const cliente=clientesCache.find(c=>Number(c.id)===Number(clienteId));if(!dados.nome)return mostrarToast('Informe o nome do serviço.');if(agendar&&(!cliente||!data||!hora))return mostrarToast('Para agendar, escolha cliente, dia e hora.');try{const servico=id?await api.atualizarServico(id,dados):await api.criarServico(dados);if(agendar)await api.criarAgendamento({cliente_id:cliente.id,cliente_nome:cliente.nome,servico_id:servico.id,servico_nome:servico.nome,data,hora,duracao_minutos:servico.duracao_minutos,valor_servico:servico.preco,status:'agendado'});fecharModal();await atualizarSistema();mostrarToast(agendar?'Serviço salvo e agendamento criado.':id?'Serviço atualizado em todo o sistema.':'Serviço cadastrado.');}catch(err){mostrarToast(err.message);}}
async function excluirServico(id){if(!confirm('Excluir este serviço e retirar também seus atendimentos e pagamentos dos totais? Essa ação não pode ser desfeita.'))return;try{await api.excluirAgendamentosDoServico(id);await api.excluirServico(id);await atualizarSistema();mostrarToast('Serviço e valores ligados a ele foram excluídos.');}catch(err){mostrarToast(err.message);}}
