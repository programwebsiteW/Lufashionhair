...
function desenharAvisosOperacionais(lista){
  ...
  const semConfirmar=lista.filter(a=>a.status==='agendado'); const proximo=proximos[0];
...
async function salvarAgendamento(id){
  ...
  if(!dados.cliente_nome||!dados.servico_nome||!dados.data)return mostrarToast('Preencha dia, cliente e serviço. O horário pode ficar em branco.');
