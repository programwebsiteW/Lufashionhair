...
async function carregarAgenda(data){
  ...
  try{agendaDiaCache=await api.listarAgendamentos(data);const visiveis=filtroStatusAgenda==='todos'?agendaDiaCache:agendaDiaCache.filter(a=>filtroStatusAgenda==='cancelado'?['cancelado','faltou'].includes(a.status):a.status===filtroStatusAgenda);document.getElementById('resumo-dia').textContent=`${visiveis.length} atendimento(s)`;lista.innerHTML=visiveis.length?visiveis.map(a=>cardAgendamento(a)).join(''):'<p class="vazio">Nenhum atendimento neste dia.</p>';}
