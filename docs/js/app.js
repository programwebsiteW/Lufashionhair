...
async function salvarServico(id){const precoTexto=document.getElementById('serv-preco').value.trim();const duracao=Number(document.getElementById('serv-horas').value||0)*60+Number(document.getElementById('serv-minutos').value||0);const dados={nome:formatarNome(document.getElementById('serv-nome').value),duracao_minutos:duracao,preco:precoTexto===''?null:parseDinheiro(precoTexto),ativo:true};const clienteId=!id?document.getElementById('serv-cliente').value:null;let cliente=clientesCache.find(c=>Number(c.id)===Number(clienteId));const data=!id?document.getElementById('serv-data').value:null;const hora=!id?document.getElementById('serv-hora').value:null;const novoNome=!id?formatarNome(document.getElementById('serv-cliente-nome').value):'';const novoTelefone=!id?document.getElementById('serv-cliente-telefone').value.trim():'';if(!dados.nome)return mostrarToast('Informe o nome do serviço.');if(duracao<1||duracao>1440)return mostrarToast('Informe um tempo entre 1 minuto e 24 horas.');if((novoNome&&!novoTelefone)||(!novoNome&&novoTelefone))return mostrarToast('Para adicionar a cliente, informe nome e WhatsApp.');try{if(novoNome&&novoTelefone){cliente=await api.criarCliente({nome:novoNome,telefone:novoTelefone});clientesCache.push(cliente);}const servico=id?await api.atualizarServico(id,dados):await api.criarServico(dados);const agendar=!id&&cliente&&data&&hora;if(agendar)await api.criarAgendamento({cliente_id:cliente.id,cliente_nome:cliente.nome,servico_id:servico.id,servico_nome:servico.nome,data,hora,duracao_minutos:servico.duracao_minutos,valor_servico:servico.preco,status:'agendado'});fecharModal();await atualizarSistema();mostrarToast(agendar?'Serviço, cliente e agendamento salvos.':id?'Serviço atualizado em todo o sistema.':novoNome?'Serviço e cliente salvos sem agendar.':'Serviço salvo sem agendar.');}catch(err){mostrarToast(err.message);}}
