...
function instanteAgendamento(a){return new Date(`${dataDoRegistro(a.data)}T${String(a.hora||'00:00').slice(0,5)}:00`);}
function estaAtrasado(a){return ['agendado','confirmado'].includes(a.status)&&instanteAgendamento(a)<new Date();}
function desenharAvisosOperacionais(lista){
  const agora=new Date(); const atrasados=lista.filter(estaAtrasado); const proximos=lista.filter(a=>instanteAgendamento(a)>=agora).sort((a,b)=>instanteAgendamento(a)-instanteAgendamento(b));
  const semConfirmar=lista.filter(a=>a.status==='agendado'&&instanteAgendamento(a)>=agora); const proximo=proximos[0];
  document.getElementById('avisos-operacionais').innerHTML=`
    ...
    <article class="aviso-operacional"><strong>${semConfirmar.length} aguardando confirmação</strong><span>${semConfirmar.length?'Use o WhatsApp para confirmar.':'Horários confirmados.'}</span></article>`;
}
...
function cardAgendamento(a,mostrarData=false){
  const atrasado=estaAtrasado(a); const status=atrasado?'Em atraso':({agendado:'Agendado',confirmado:'Confirmado',em_andamento:'Em andamento',concluido:'Concluído',cancelado:'Cancelado',faltou:'Cliente faltou'}[a.status]||a.status);
  const pagamento=a.pagamento; const pagTexto=pagamento?.status==='pago'?'Pago':pagamento?.status==='parcial'?'Parcial':'Pendente';
  return `<article class="item-card ${atrasado?'atrasado':escapar(a.status)}">
    <div class="hora-destaque">${mostrarData?escapar(dataDoRegistro(a.data).slice(8,10)+'/'+dataDoRegistro(a.data).slice(5,7)):escapar(String(a.hora).slice(0,5))}</div>
    <div class="item-info"><strong>${escapar(a.cliente_nome)}</strong><span>${escapar(a.servico_nome)} · ${duracaoTexto(a.duracao_minutos)} · ${dinheiro(a.valor_servico)}</span>${a.observacoes?`<span>${escapar(a.observacoes)}</span>`:''}<div class="tags"><span class="tag ${atrasado?'atrasado':escapar(a.status)}">${escapar(status)}</span><span class="tag pagamento-${pagamento?.status||'pendente'}">${pagTexto}</span></div></div>
    <div class="acoes">${a.cliente_telefone?`<button onclick="abrirWhatsAppAgendamento(${Number(a.id)})">WhatsApp</button>`:''}<button onclick="editarAgendamento(${Number(a.id)})">Editar</button><button onclick="abrirPagamento(${Number(a.id)})">Pagamento</button>${a.status==='agendado'?`<button onclick="marcarStatus(${Number(a.id)},'confirmado')">Confirmar</button>`:''}${['agendado','confirmado'].includes(a.status)?`<button onclick="marcarStatus(${Number(a.id)},'em_andamento')">Iniciar</button>`:''}${['agendado','confirmado','em_andamento'].includes(a.status)?`<button onclick="marcarStatus(${Number(a.id)},'concluido')">Concluir</button><button onclick="marcarStatus(${Number(a.id)},'faltou')">Faltou</button><button onclick="marcarStatus(${Number(a.id)},'cancelado')">Cancelar</button>`:''}</div>
  </article>`;
}
async function carregarAgenda(data){
  ...
  try{agendaDiaCache=await api.listarAgendamentos(data);const visiveis=filtroStatusAgenda==='todos'?agendaDiaCache:agendaDiaCache.filter(a=>a.status===filtroStatusAgenda);document.getElementById('resumo-dia').textContent=`${visiveis.length} atendimento(s)`;lista.innerHTML=visiveis.length?visiveis.map(a=>cardAgendamento(a)).join(''):'<p class="vazio">Nenhum atendimento neste dia.</p>';}
...
function formularioAgendamento(item={}){
  ...
  abrirModal(item.id?'Editar agendamento':'Novo agendamento',`
    ...
    <div class="campo"><label for="ag-servico-id">3. Serviço</label><select id="ag-servico-id"><option value="">Escolha um serviço</option>${servicos}</select></div>
  ...
  const cli=document.getElementById('ag-cliente-id'); const serv=document.getElementById('ag-servico-id');
  ...
  const resumo=()=>{const s=servicosCache.find(x=>Number(x.id)===Number(serv.value));document.getElementById('ag-resumo-servico').innerHTML=s?`Duração prevista: <strong>${duracaoTexto(s.duracao_minutos)}</strong> · Valor: <strong>${s.preco==null?'a definir':dinheiro(s.preco)}</strong>`:'';};serv.addEventListener('change',resumo);resumo();
...
async function abrirNovoAgendamento(clienteId=null,servicoId=null){
  ...
  if(!servicosCache.length){mostrarToast('Cadastre pelo menos um serviço antes de agendar.');return irParaTela('servicos');}
...
async function salvarAgendamento(id){
  const clienteId=document.getElementById('ag-cliente-id').value;const cliente=clientesCache.find(c=>Number(c.id)===Number(clienteId));const servicoId=document.getElementById('ag-servico-id').value;const servico=servicosCache.find(s=>Number(s.id)===Number(servicoId));
  const dados={cliente_id:clienteId||null,cliente_nome:formatarNome(document.getElementById('ag-cliente-nome').value)||cliente?.nome,servico_id:servicoId||null,servico_nome:servico?.nome,data:document.getElementById('ag-data').value,hora:document.getElementById('ag-hora').value,duracao_minutos:servico?.duracao_minutos||60,valor_servico:servico?.preco??null,observacoes:document.getElementById('ag-obs').value.trim(),status:id?(localizarAgendamento(id)?.status||'agendado'):'agendado'};
  if(!dados.cliente_nome||!dados.servico_nome||!dados.data||!dados.hora)return mostrarToast('Preencha dia, hora, cliente e serviço.');
  try{if(id)await api.atualizarAgendamento(id,dados);else await api.criarAgendamento(dados);fecharModal();diaSelecionado=dados.data;const[ano,mes]=dados.data.split('-').map(Number);mesAtual=new Date(ano,mes-1,1);await carregarCalendario();await carregarAgenda(dados.data);await carregarInicio();mostrarToast(id?'Agendamento atualizado.':'Agendamento criado e pagamento preparado.');}catch(err){mostrarToast(err.message);}
}
...
function abrirWhatsAppAgendamento(id){const a=localizarAgendamento(id);const tel=telefoneWhatsApp(a?.cliente_telefone);if(!tel)return mostrarToast('Cadastre o telefone da cliente primeiro.');const msg=`Olá, ${a.cliente_nome}! Confirmando seu horário de ${a.servico_nome} no dia ${formatarData(a.data)} às ${String(a.hora).slice(0,5)}. Lu Fashion Hair.`;window.open(`https://wa.me/${tel}?text=${encodeURIComponent(msg)}`,'_blank','noopener');}
...
function abrirPagamento(id){
  ...
  abrirModal('Registrar pagamento',`<div class="resumo-pagamento"><strong>${escapar(a.cliente_nome)}</strong><span>${escapar(a.servico_nome)} · ${formatarData(a.data)}</span></div>
    ...
    <div class="modal-botoes"><button class="btn-cancelar" onclick="fecharModal()">Voltar</button><button class="btn-confirmar" onclick="salvarPagamento(${Number(p.id)})">Salvar pagamento</button></div>`);
}
async function salvarPagamento(id){
  if(!id)return mostrarToast('Pagamento ainda está sendo preparado. Feche e abra novamente.');
  ...
  try{await api.atualizarPagamento(id,dados);fecharModal();await carregarCalendario();await carregarAgenda(diaSelecionado);await carregarInicio();await carregarFinanceiro();mostrarToast('Pagamento salvo.');}catch(err){mostrarToast(err.message);}
}
...
async function salvarCliente(id){const dados={nome:formatarNome(document.getElementById('cli-nome').value),telefone:document.getElementById('cli-telefone').value.trim()||null,observacoes:document.getElementById('cli-obs').value.trim()||null};if(!dados.nome)return mostrarToast('Informe o nome da cliente.');try{if(id)await api.atualizarCliente(id,dados);else await api.criarCliente(dados);fecharModal();await carregarClientes();await carregarCalendario();mostrarToast(id?'Cliente atualizada em todo o sistema.':'Cliente cadastrada.');}catch(err){mostrarToast(err.message);}}
...
function abrirWhatsAppCliente(id){const c=clientesCache.find(x=>Number(x.id)===Number(id));const tel=telefoneWhatsApp(c?.telefone);if(tel)window.open(`https://wa.me/${tel}`,'_blank','noopener');}
...
function formularioServico(s={}){abrirModal(s.id?'Editar serviço':'Novo serviço',`${!s.id?`<div class="campo"><label for="serv-modelo">Começar com um modelo</label><select id="serv-modelo"><option value="">Escolha ou preencha abaixo</option>${Object.entries(modelosServico).map(([k,v])=>`<option value="${k}">${v.nome}</option>`).join('')}</select></div>`:''}<div class="campo"><label for="serv-nome">Nome do serviço</label><input id="serv-nome" value="${escapar(s.nome||'')}" placeholder="Ex.: Progressiva"></div><div class="linha-campos"><div class="campo"><label for="serv-duracao">Tempo necessário</label><select id="serv-duracao">${opcoesDuracao(s.duracao_minutos||60)}</select></div><div class="campo"><label for="serv-preco">Valor</label><div class="campo-dinheiro"><span>R$</span><input id="serv-preco" inputmode="decimal" value="${valorCampo(s.preco)}" placeholder="Deixe em branco e digite"></div></div></div><p class="ajuda-campo">A duração aparece de forma simples para a Lu, mas o sistema organiza os horários internamente.</p><div class="modal-botoes"><button class="btn-cancelar" onclick="fecharModal()">Voltar</button><button class="btn-confirmar" onclick="salvarServico(${s.id?Number(s.id):'null'})">Salvar</button></div>`);document.getElementById('serv-modelo')?.addEventListener('change',e=>{const m=modelosServico[e.target.value];if(m){document.getElementById('serv-nome').value=m.nome;document.getElementById('serv-duracao').value=m.duracao;}});document.getElementById('serv-nome').addEventListener('blur',e=>e.target.value=formatarNome(e.target.value));}
...
async function salvarServico(id){const precoTexto=document.getElementById('serv-preco').value.trim();const dados={nome:formatarNome(document.getElementById('serv-nome').value),duracao_minutos:Number(document.getElementById('serv-duracao').value),preco:precoTexto===''?null:parseDinheiro(precoTexto),ativo:true};if(!dados.nome)return mostrarToast('Informe o nome do serviço.');try{if(id)await api.atualizarServico(id,dados);else await api.criarServico(dados);fecharModal();await carregarServicos();await carregarCalendario();mostrarToast(id?'Serviço atualizado nos próximos agendamentos.':'Serviço cadastrado.');}catch(err){mostrarToast(err.message);}}
async function excluirServico(id){if(!confirm('Excluir este serviço da lista? Agendamentos antigos continuarão salvos.'))return;try{await api.excluirServico(id);await carregarServicos();mostrarToast('Serviço excluído.');}catch(err){mostrarToast(err.message);}}

document.getElementById('financeiro-mes').addEventListener('change',carregarFinanceiro);document.getElementById('resumo-mes').addEventListener('change',carregarResumos);
async function carregarFinanceiro(){
  const {inicio,fim}=periodoMes(document.getElementById('financeiro-mes').value);try{[pagamentosCache,despesasCache]=await Promise.all([api.listarPagamentosPeriodo(inicio,fim),api.listarDespesasPeriodo(inicio,fim)]);pagamentosCache=pagamentosCache.filter(p=>p.agendamentos);const recebido=pagamentosCache.reduce((s,p)=>s+Number(p.valor_pago||0),0);const pendente=pagamentosCache.filter(p=>!['cancelado','faltou'].includes(p.agendamentos.status)).reduce((s,p)=>s+pendentePagamento(p),0);const despesas=despesasCache.filter(d=>d.pago).reduce((s,d)=>s+Number(d.valor),0);document.getElementById('fin-recebido').textContent=dinheiro(recebido);document.getElementById('fin-pendente').textContent=dinheiro(pendente);document.getElementById('fin-despesas').textContent=dinheiro(despesas);document.getElementById('fin-saldo').textContent=dinheiro(recebido-despesas);desenharPagamentos();desenharDespesas();}catch(err){document.getElementById('lista-pagamentos').innerHTML=`<p class="vazio">${escapar(err.message)}</p>`;}
}
function desenharPagamentos(){document.getElementById('lista-pagamentos').innerHTML=pagamentosCache.length?pagamentosCache.map(p=>`<article class="item-card"><div class="item-info"><strong>${escapar(p.agendamentos.cliente_nome)}</strong><span>${formatarData(p.agendamentos.data)} · ${escapar(p.agendamentos.servico_nome)}</span><span>${p.status==='pago'?`${dinheiro(p.valor_pago)} recebido por ${escapar(p.forma_pagamento||'')}`:`${dinheiro(pendentePagamento(p))} pendente`}</span><span class="tag pagamento-${p.status}">${p.status==='pago'?'Pago':p.status==='parcial'?'Parcial':'Pendente'}</span></div><div class="acoes"><button onclick="abrirPagamentoFinanceiro(${p.id})">${p.status==='pago'?'Editar':'Receber'}</button></div></article>`).join(''):'<p class="vazio">Nenhum pagamento neste mês.</p>';}
...
async function salvarDespesa(id){const dados={descricao:formatarNome(document.getElementById('desp-descricao').value),valor:parseDinheiro(document.getElementById('desp-valor').value),data:document.getElementById('desp-data').value,categoria:document.getElementById('desp-categoria').value,pago:document.getElementById('desp-pago').checked,observacoes:document.getElementById('desp-obs').value.trim()||null};if(!dados.descricao||dados.valor<=0)return mostrarToast('Informe a descrição e um valor válido.');try{if(id)await api.atualizarDespesa(id,dados);else await api.criarDespesa(dados);fecharModal();await carregarFinanceiro();mostrarToast('Despesa salva.');}catch(err){mostrarToast(err.message);}}
async function excluirDespesa(id){if(!confirm('Excluir esta despesa?'))return;try{await api.excluirDespesa(id);await carregarFinanceiro();mostrarToast('Despesa excluída.');}catch(err){mostrarToast(err.message);}}

async function carregarResumos(){
  const {inicio,fim}=periodoMes(document.getElementById('resumo-mes').value);try{const[agenda,pagamentos,despesas]=await Promise.all([api.listarAgendamentosPeriodo(inicio,fim),api.listarPagamentosPeriodo(inicio,fim),api.listarDespesasPeriodo(inicio,fim)]);const validos=agenda.filter(a=>!['cancelado','faltou'].includes(a.status));const concluidos=agenda.filter(a=>a.status==='concluido');const recebidos=pagamentos.reduce((s,p)=>s+Number(p.valor_pago||0),0);const gastos=despesas.filter(d=>d.pago).reduce((s,d)=>s+Number(d.valor),0);const faltas=agenda.filter(a=>a.status==='faltou').length;const cancelados=agenda.filter(a=>a.status==='cancelado').length;document.getElementById('resumo-cards').innerHTML=`<article class="metrica"><span>Atendimentos</span><strong>${validos.length}</strong><small>${concluidos.length} concluídos</small></article><article class="metrica destaque-verde"><span>Recebido</span><strong>${dinheiro(recebidos)}</strong><small>ticket médio ${dinheiro(concluidos.length?recebidos/concluidos.length:0)}</small></article><article class="metrica"><span>Despesas</span><strong>${dinheiro(gastos)}</strong><small>saldo ${dinheiro(recebidos-gastos)}</small></article><article class="metrica"><span>Faltas e cancelamentos</span><strong>${faltas+cancelados}</strong><small>${faltas} faltas · ${cancelados} cancelados</small></article>`;desenharRanking('resumo-servicos',validos,'servico_nome');desenharRanking('resumo-clientes',validos,'cliente_nome');}catch(err){document.getElementById('resumo-cards').innerHTML=`<p class="vazio">${escapar(err.message)}</p>`;}
}
